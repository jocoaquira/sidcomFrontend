...
@Component({
    templateUrl: './listar-presentaciones.component.html',
    providers: [MessageService]
})
export class ListarPresentacionComponent implements OnInit {

    public listaPresentaciones!:IPresentacion[];
    ...
    findIndexById(id: number): number {
        let index = -1;
        for (let i = 0; i < this.listaPresentaciones.length; i++) {
            if (this.listaPresentaciones[i].id === id) {
                index = i;
                break;
            }
        }
    ...
    bloquearDialogo(presentacion:IPresentacion){
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                this.presentacionService.editarpresentacion(presentacion).subscribe(
                    (data:any) =>
                    {
