...
@Component({
    templateUrl: './listar-minerales.component.html',
    styleUrls: ['./listar-minerales.component.scss'],
    providers: [MessageService]
})
export class ListarMineralComponent implements OnInit {

    public listaMinerales!:IMineral[];
    ...
    findIndexById(id: number): number {
        let index = -1;
        for (let i = 0; i < this.listaMinerales.length; i++) {
            if (this.listaMinerales[i].id === id) {
                index = i;
                break;
            }
        }
    ...
    bloquearDialogo(mineral:IMineral){
        
        this.confirmationService.confirm({
            key: 'confirm1',
            message: '¿Estas seguro de Realizar esta Operación?',
            accept: () => {
                if(mineral.estado=='ACTIVO')
                    {
                        mineral.estado='INACTIVO';
                    }
                    else{
                        mineral.estado='ACTIVO';
                    }
                this.mineralService.editarmineral(mineral).subscribe(
                    (data:any) =>
                    {
