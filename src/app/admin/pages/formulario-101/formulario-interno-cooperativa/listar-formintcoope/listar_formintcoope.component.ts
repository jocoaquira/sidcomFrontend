...
@Component({
  selector: 'app-listar_formintcoope',
  templateUrl: './listar_formintcoope.component.html',
  styleUrls: ['./listar_formintcoope.component.scss']
})
export class ListarFormularioCooperativaComponent implements OnInit {

    @ViewChild('dt') dt!: Table;
    public error!:any;
    listaFormularioCooperativas: IFormularioInternoSimple[] = [];
    cols: any[] = [];
    ...
    loadData() {
        ...
        this.formIntService.getFormReducidoOptimizado(
          ...
          next: (response) => {
            this.listaFormularioCooperativas = response.data;
    ...
    emitir(event:IFormularioInternoSimple){
            ...
            this.formIntService.emitirFormularioInterno(event.id).subscribe(
                (data:any)=>{
                    ...
                    {
                        let index = this.listaFormularioCooperativas.findIndex(i => i.id === formulario_emitido.id);
                        if (index !== -1) {
                            this.listaFormularioCooperativas[index].estado = formulario_emitido.estado; // Actualizamos el valor
                            this.listaFormularioCooperativas[index].fecha_creacion=formulario_emitido.fecha_creacion;
                            this.listaFormularioCooperativas[index].fecha_vencimiento=formulario_emitido.fecha_vencimiento;
                            this.listaFormularioCooperativas[index].nro_formulario=formulario_emitido.nro_formulario;
                        }
